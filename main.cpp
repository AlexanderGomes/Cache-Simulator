...
bool cacheSim(unsigned int addr, int cache[3][100000], int states[100000],
              int type, int &block_counter, int index_addr, int tag_addr,
              int core_id, int states_other_core[100000]) {
  int shift_offset = std::bit_width(block_size - 1);
  bool detected = false;
      ...
      if (block_counter < number_of_blocks) {
        ...
        if (!detected) {
          ...
          for (int i = 1; i < number_of_blocks; i++) {
            if (cache[1][compare] > cache[1][i]) compare = i;
          }
...
int main(int argc, const char *argv[]) {
  ...
  int cache[2][3][100000];         // 2 cores, 3 arrays for each cache
  ...
  for (int core = 0; core < 2; core++) {
    if (cache_type == 0) {  // Direct Mapped
      ...
      for (int i = 0; i < 2; i++)  // Initialize cache with -1
        for (int j = 0; j < number_of_blocks; j++) cache[core][i][j] = -1;
      for (int j = 0; j < number_of_blocks; j++) states[core][j] = INVALID;

      for (int i = 0; i < looper; i++) {
        addr = memGen1();
        shift = std::bit_width(block_size - 1);
        index_addr = (addr >> shift) % number_of_blocks;
        shift = std::bit_width(number_of_blocks + block_size - 1);
        tag_addr = addr >> shift;  // Calculate index and tag
        flag = cacheSim(addr, cache[core], states[core], 0, block_counter[core],
      ...
      cout << "Core " << core << " Hits: " << hit_counter[core] << endl
           << "Compulsory: " << compulsory_misses << endl
           << "Capacity: " << capacity_misses << endl
      ...
      for (int i = 0; i < 3; i++)  // Initialize cache with -1
        for (int j = 0; j < 100000; j++) cache[core][i][j] = -1;
      for (int j = 0; j < number_of_blocks; j++) states[core][j] = INVALID;
      ...
      for (int i = 0; i < looper; i++) {
        ...
        shift = std::bit_width(number_of_blocks + block_size - 1);
        tag_addr = addr >> shift;  // Calculate index and tag
        flag = cacheSim(addr, cache[core], states[core], number_of_ways,
      ...
      for (int i = 0; i < looper; i++) {
        addr = memGen4();
        flag = cacheSim(addr, cache[core], states[core], replacement_type,
                        block_counter[core], index_addr, tag_addr, core,
                        states[(core + 1) % 2]);
        cout << "0x" << setfill('0') << setw(8) << hex << addr << " ("
             << msg[flag] << ") Core " << core << "\n";
